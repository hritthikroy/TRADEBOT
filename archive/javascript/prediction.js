...
let chart = null; // Main chart
let candleSeries = null;
let priceLinesMap = {};
let predictionChart = null; // Prediction overlay chart
let predictionCandleSeries = null;
let predictionCandleSeriesFuture = null; // For predicted candles
let predictionPriceLines = {};
let currentData = [];
let canvas;
let ctx;
...
window.currentInterval = '15m'; // Set global for signal generation
...
async function saveSignalToTracker(signal) {
    ...
    // To enable: Remove the return statement below
    console.log('⛔ Signal saving DISABLED - Use Go backend server to generate signals');
    return;
        ...
        if (typeof SupabaseDB !== 'undefined' && typeof window.supabase !== 'undefined') {
            try {
                ...
                const result = await SupabaseDB.saveSignal({
                    ...
                    strength: signalData.strength,
                    status: 'pending',
                    timeframe: signalData.timeframe || window.currentInterval || '15m'  // Add timeframe
                });
...
function initChart() {
    ...
    console.log('Initializing Lightweight Chart, container:', container);
...
async function loadChartData(symbol = 'BTCUSDT', interval = '15m') {
    try {
        const binanceInterval = toBinanceInterval(interval);
        const response = await fetch(`https://api.binance.com/api/v3/klines?symbol=${symbol}&interval=${binanceInterval}&limit=1000`);
        const data = await response.json();
        ...
        console.log(`Loaded ${candles.length} candles for ${symbol} ${interval}`);
    } catch (error) {
...
function initCanvas() {
    ...
    try {
        if (typeof LightweightCharts === 'undefined') {
            console.error('LightweightCharts library not loaded for prediction chart');
            return;
        }
        ...
        predictionCandleSeries = predictionChart.addCandlestickSeries({
            upColor: '#26a69a',
            downColor: '#ef5350',
            borderVisible: false,
            wickUpColor: '#26a69a',
        ...
        // Add second series for predicted candles (white/transparent)
        predictionCandleSeriesFuture = predictionChart.addCandlestickSeries({
            upColor: 'rgba(255, 255, 255, 0.3)',
...
async function fetchMultiTimeframeData() {
    try {
        ...
        const promises = timeframes.map(tf => {
            const binanceTF = toBinanceInterval(tf);
            return fetch(`https://api.binance.com/api/v3/klines?symbol=${currentSymbol}&interval=${binanceTF}&limit=100`)
                .then(res => res.json())
                .then(data => {
                    // Check for API error
                    if (data.code) {
                        console.warn(`Binance API Error for ${tf}:`, data.msg);
...
function drawChart() {
    if (!predictionCandleSeries || !predictionCandleSeriesFuture) {
        console.log('Chart series not initialized');
        return;
    }
    
    // Convert currentData to Lightweight Charts format
    if (currentData.length === 0) {
        console.log('No current data to display');
        return;
    }
    
    // Real candles (historical data)
    const realCandles = currentData.map(d => ({
    ...
    predictionCandleSeries.setData(realCandles);
    console.log(`📊 Drew ${realCandles.length} real candles`);
    
    // Predicted candles (future predictions) - shown in white/transparent
    if (predictedCandles && predictedCandles.length > 0) {
        const futureCandles = predictedCandles.map(pred => ({
        ...
        predictionCandleSeriesFuture.setData(futureCandles);
        console.log(`🔮 Drew ${futureCandles.length} predicted candles:`, futureCandles);
        
        // Update prediction count
        const countEl = document.getElementById('prediction-count');
        if (countEl) {
            countEl.textContent = `${predictedCandles.length} Predictions`;
        }
    } else {
        // Clear predicted candles if none
        predictionCandleSeriesFuture.setData([]);
        console.log('No predicted candles to display');
    }
    
    // Draw signal lines if available
    drawSignalLines();
}

// Draw signal lines on main background chart
function drawMainChartSignalLines(signal) {
    console.log('🎯 drawMainChartSignalLines called with signal:', signal);
    console.log('🎯 candleSeries exists:', !!candleSeries);
    
    if (!candleSeries) {
        console.error('❌ candleSeries not initialized - cannot draw lines');
        return;
    }
    
    if (!signal) {
        console.error('❌ No signal provided');
        return;
    }
    
    try {
        // Remove existing lines from main chart
        ...
        console.log(`📍 Drawing lines: Entry=${signal.entry}, SL=${signal.stopLoss}, TP=${signal.takeProfit}`);
        ...
        console.log('✅ Entry line created');
        ...
        if (signal.tp3) {
            ...
            console.log('✅ TP3 line created');
        }
        
        console.log(`✅ Successfully drew ${Object.keys(priceLinesMap).length} signal lines on main chart`);
    } catch (error) {
        console.error('❌ Error drawing signal lines:', error);
    }
}

// Draw signal lines on prediction chart
function drawSignalLines() {
    if (!predictionCandleSeries || !historicalSignals) return;
    
    // Remove existing lines
    Object.values(predictionPriceLines).forEach(line => {
        predictionCandleSeries.removePriceLine(line);
    });
    predictionPriceLines = {};
    
    // Draw lines for recent signals
    historicalSignals.slice(-3).forEach((signal, index) => {
        ...
        // Entry line
        predictionPriceLines[`entry_${index}`] = predictionCandleSeries.createPriceLine({
            price: signal.entry,
            color: color,
            lineWidth: 2,
            lineStyle: LightweightCharts.LineStyle.Solid,
        ...
        // Stop Loss
        if (signal.stopLoss) {
            predictionPriceLines[`sl_${index}`] = predictionCandleSeries.createPriceLine({
                price: signal.stopLoss,
                color: '#ef5350',
                lineWidth: 1,
                lineStyle: LightweightCharts.LineStyle.Dashed,
        ...
        // Take Profit
        if (signal.takeProfit) {
            predictionPriceLines[`tp_${index}`] = predictionCandleSeries.createPriceLine({
                price: signal.takeProfit,
                color: '#26a69a',
                lineWidth: 1,
                lineStyle: LightweightCharts.LineStyle.Dashed,
...
async function autoPredictNextCandles() {
    ...
    if (window.generateTradingSignal && window.displayTradingSignal) {
        ...
        // Log signal with timeframe tag
        if (tradingSignal) {
            const tf = tradingSignal.timeframe || window.currentInterval || '15m';
            const tp1 = tradingSignal.targets && tradingSignal.targets[0] ? tradingSignal.targets[0].price.toFixed(2) : 'N/A';
            const rr = tradingSignal.bestRR ? tradingSignal.bestRR.toFixed(2) : (tradingSignal.riskReward ? tradingSignal.riskReward.toFixed(2) : 'N/A');
            console.log(`🎯 [${tf.toUpperCase()}] Trading signal generated:`, tradingSignal.type, 
                       `| Entry: ${tradingSignal.entry.toFixed(2)}`,
                       `| SL: ${tradingSignal.stopLoss.toFixed(2)}`,
                       `| TP1: ${tp1}`,
                       `| RR: ${rr}:1`,
                       `| Confluence: ${tradingSignal.confluence || 'N/A'}`);
        } else {
        ...
        if (tradingSignal && shouldUpdateSignal) {
            ...
            // Draw signal lines on main chart
            drawMainChartSignalLines(tradingSignal);
...
function changeInterval(interval) {
    ...
    window.currentInterval = interval;  // Set global for signal generation
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    setTimeout(() => {
        ...
        // Start simulation after charts are ready
        setTimeout(() => {
            try {
                startSimulation();
