...
function generateTradingSignal(data, aiPrediction, volumeAnalysis, mtfTrendFilter, useEnhancedICT = true) {
    if (data.length < 20) return null;
    ...
    // 🎯 PRIORITY: Try enhanced ICT/SMC entry models first (low risk, high reward)
    // Disabled for backtest due to unrealistic returns
    if (useEnhancedICT) {
        try {
            const enhancedSignal = generateEnhancedICTSignal(data, aiPrediction, volumeAnalysis, mtfTrendFilter);
            if (enhancedSignal && enhancedSignal.entry && enhancedSignal.stopLoss) {
                console.log('✨ Using Enhanced ICT Entry Model:', enhancedSignal.reason || 'High Quality Setup');
                return enhancedSignal;
            }
        } catch (error) {
            console.warn('Enhanced ICT signal error, falling back to standard:', error.message);
        }
    }
    ...
    if (isBullish && signalStrength >= 55) {
        ...
        // 🚀 DYNAMIC CONFLUENCE: Adjust based on timeframe
        // Lower timeframes (1m-15m): Need more confluence (more noise)
        // Higher timeframes (1h-4h): Need less confluence (cleaner signals)
        const timeframeConfluenceMap = {
            '1m': 10,   // Higher requirement (noisy)
            '3m': 9,
            '5m': 9,
            '15m': 8,   // Balanced
            '30m': 7,
            '1h': 6,    // Lower requirement (cleaner)
            '2h': 5,
            '4h': 5,
            '6h': 4,
            '8h': 4,
        ...
        const minConfluence = timeframeConfluenceMap[currentTF] || 8;
        ...
        const hasPremiumSetup = confluence >= (minConfluence + 4) && hasStrongConfirmation;
        const hasQualitySetup = confluence >= minConfluence;
        
        if (hasPremiumSetup || hasQualitySetup) {
            ...
            // 🚀 DYNAMIC TARGETS: Adjust based on timeframe
            // Higher timeframes = Bigger moves = Bigger targets
            const timeframeMultiplierMap = {
                '1m': 0.8,   // Smaller moves
                '3m': 0.9,
                '5m': 1.0,
                '15m': 1.1,
                '30m': 1.2,
            ...
            const tfMultiplier = timeframeMultiplierMap[currentTF] || 1.0;
            const premiumBonus = hasPremiumSetup ? 1.3 : 1.0;
            const finalMultiplier = tfMultiplier * premiumBonus;
            
            const takeProfit1 = currentPrice + (atr * 3.0 * finalMultiplier);
            const takeProfit2 = currentPrice + (atr * 5.0 * finalMultiplier);
            const takeProfit3 = currentPrice + (atr * 8.0 * finalMultiplier);
    ...
    // ICT/SMC SELL SETUP
        ...
        if (hasPremiumSetup || hasQualitySetup) {
            ...
            // 🚀 AGGRESSIVE TARGETS: Higher risk:reward for maximum profit
            // Premium setups get bigger targets
            const targetMultiplier = hasPremiumSetup ? 1.3 : 1.0;
            const takeProfit1 = currentPrice - (atr * 3.0 * targetMultiplier);
            const takeProfit2 = currentPrice - (atr * 5.0 * targetMultiplier);
            const takeProfit3 = currentPrice - (atr * 8.0 * targetMultiplier);
...
function drawTradingSignalOnChart(signal, leftPadding, topPadding, chartHeight, maxPrice, minPrice, padding) {
    ...
    ctx.fillText(`${signal.entry ? signal.entry.toFixed(2) : 'N/A'}`, leftPadding + chartWidth + 10, entryY + 11);
    ...
    ctx.fillText(`${signal.stopLoss ? signal.stopLoss.toFixed(2) : 'N/A'}`, leftPadding + chartWidth + 10, slY + 11);
    ...
    signal.targets.forEach((target, i) => {
        ...
        ctx.font = 'bold 11px Arial';
        ctx.textAlign = 'left';
        ctx.fillText(`TP${i+1} (${target.rr ? target.rr.toFixed(1) : 'N/A'}R)`, leftPadding + chartWidth + 10, tpY - 1);
        ctx.fillText(`${target.price ? target.price.toFixed(2) : 'N/A'}`, leftPadding + chartWidth + 10, tpY + 11);
    });
    ...
    ctx.font = '12px Arial';
    ctx.fillText(`Entry: $${signal.entry ? signal.entry.toFixed(2) : 'N/A'}`, boxX + 10, boxY + 45);
    ctx.fillText(`SL: $${signal.stopLoss ? signal.stopLoss.toFixed(2) : 'N/A'}`, boxX + 10, boxY + 60);
    ctx.fillText(`RR: ${signal.bestRR ? signal.bestRR.toFixed(1) : 'N/A'}:1 | ${signal.strength || 0}%`, boxX + 10, boxY + 75);
}
        ...
        <div style="background: ${color}22; border-left: 4px solid ${color}; padding: 15px; border-radius: 4px; margin-top: 10px;">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-top: 10px;">
                <div>
                    ...
                    <div style="color: ${color}; font-size: 16px; font-weight: bold;">
                        $${signal.entry ? signal.entry.toFixed(2) : 'N/A'}
                    </div>
                </div>
                ...
                ${signal.targets.map((target, i) => `
                    <div style="display: flex; justify-content: space-between; margin: 5px 0; font-size: 13px;">
                        ...
                        <span style="color: #26a69a; font-weight: bold;">
                            $${target.price ? target.price.toFixed(2) : 'N/A'}
                            <span style="color: #787b86; font-size: 11px; margin-left: 5px;">
                                RR: ${target.rr ? target.rr.toFixed(2) : 'N/A'}:1
                            </span>
                        </span>
                    </div>
                `).join('')}
            ...
            <div style="margin-top: 10px; padding-top: 10px; border-top: 1px solid #2a2e39; display: flex; justify-content: space-between; font-size: 12px;">
                <div>
                    ...
                    <span style="color: #ef5350; font-weight: bold;">${signal.riskPercent ? signal.riskPercent.toFixed(2) : 'N/A'}%</span>
                </div>
                <div>
                    <span style="color: #787b86;">Best RR:</span>
                    <span style="color: #26a69a; font-weight: bold;">${signal.bestRR ? signal.bestRR.toFixed(2) : (signal.targets && signal.targets[0] && signal.targets[0].rr ? signal.targets[0].rr.toFixed(2) : 'N/A')}:1</span>
                </div>
                <div>
                    <span style="color: #787b86;">ATR:</span>
                    <span style="color: #2962ff; font-weight: bold;">$${signal.atr ? signal.atr.toFixed(2) : 'N/A'}</span>
                </div>
            </div>
        </div>
    `;
...
// ═══════════════════════════════════════════════════════════
// ADVANCED ICT/SMC ENTRY MODELS - LOW RISK, HIGH REWARD
// ═══════════════════════════════════════════════════════════

// 1. OPTIMAL TRADE ENTRY (OTE) - Fibonacci 0.62-0.79 retracement
function findOptimalTradeEntry(data, orderBlocks) {
    if (data.length < 20) return null;
    ...
    // Calculate OTE zones (0.62-0.79 Fibonacci retracement)
    const bullishOTE = {
        entry: swingLow + (range * 0.705), // Sweet spot at 0.705
        min: swingLow + (range * 0.62),
        max: swingLow + (range * 0.79),
        stopLoss: swingLow - (range * 0.05),
        target: swingHigh + (range * 0.5)
    };
    
    const bearishOTE = {
        entry: swingHigh - (range * 0.705),
        min: swingHigh - (range * 0.79),
        max: swingHigh - (range * 0.62),
        stopLoss: swingHigh + (range * 0.05),
        target: swingLow - (range * 0.5)
    };
    
    // Check if price is in OTE zone
    const inBullishOTE = currentPrice >= bullishOTE.min && currentPrice <= bullishOTE.max;
    const inBearishOTE = currentPrice >= bearishOTE.min && currentPrice <= bearishOTE.max;
    
    return {
        bullish: inBullishOTE ? bullishOTE : null,
        bearish: inBearishOTE ? bearishOTE : null,
        swingHigh: swingHigh,
        swingLow: swingLow
    };
}

// 2. BREAKER BLOCK ENTRY - Failed support becomes resistance (and vice versa)
function findBreakerBlockEntry(data, orderBlocks) {
    const breakerEntries = { bullish: [], bearish: [] };
    
    if (!orderBlocks || data.length < 20) return breakerEntries;
    
    const currentPrice = data[data.length - 1].close;
    const recent = data.slice(-10);
    
    // Bullish Breaker: Failed resistance that price broke above
    orderBlocks.bearish.forEach(ob => {
        const priceAbove = recent.some(c => c.close > ob.high);
        const nowRetesting = currentPrice >= ob.low * 0.995 && currentPrice <= ob.high * 1.005;
        
        if (priceAbove && nowRetesting) {
            breakerEntries.bullish.push({
                entry: (ob.high + ob.low) / 2,
                stopLoss: ob.low - (ob.high - ob.low) * 0.2,
                target: ob.high + (ob.high - ob.low) * 3,
                riskReward: 3.5,
                confidence: 85
            });
        }
    ...
    // Bearish Breaker: Failed support that price broke below
    orderBlocks.bullish.forEach(ob => {
        const priceBelow = recent.some(c => c.close < ob.low);
        const nowRetesting = currentPrice >= ob.low * 0.995 && currentPrice <= ob.high * 1.005;
        
        if (priceBelow && nowRetesting) {
            breakerEntries.bearish.push({
                entry: (ob.high + ob.low) / 2,
                stopLoss: ob.high + (ob.high - ob.low) * 0.2,
                target: ob.low - (ob.high - ob.low) * 3,
                riskReward: 3.5,
                confidence: 85
            });
        }
...
// 3. MITIGATION BLOCK ENTRY - Price returning to unfilled orders
function findMitigationBlockEntry(data) {
    if (data.length < 30) return { bullish: [], bearish: [] };
    
    const mitigationBlocks = { bullish: [], bearish: [] };
    const currentPrice = data[data.length - 1].close;
    const recent = data.slice(-30);
    
    // Find strong moves that left gaps
    for (let i = 5; i < recent.length - 5; i++) {
        const candle = recent[i];
        const prevCandle = recent[i - 1];
        const moveSize = Math.abs(candle.close - candle.open);
        const avgSize = recent.slice(i - 5, i).reduce((sum, c) => sum + Math.abs(c.close - c.open), 0) / 5;
        
        // Strong bullish move (potential bearish mitigation)
        if (moveSize > avgSize * 2 && candle.close > candle.open) {
            const mitigationZone = {
                high: candle.open + moveSize * 0.3,
                low: candle.open,
                entry: candle.open + moveSize * 0.15,
                stopLoss: candle.open - moveSize * 0.2,
                target: candle.close + moveSize * 1.5,
                riskReward: 4.0
            };
            
            // Check if price is near this zone
            if (currentPrice >= mitigationZone.low * 0.99 && currentPrice <= mitigationZone.high * 1.01) {
                mitigationBlocks.bearish.push(mitigationZone);
            }
        }
        
        // Strong bearish move (potential bullish mitigation)
        if (moveSize > avgSize * 2 && candle.close < candle.open) {
            const mitigationZone = {
                high: candle.open,
                low: candle.open - moveSize * 0.3,
                entry: candle.open - moveSize * 0.15,
                stopLoss: candle.open + moveSize * 0.2,
                target: candle.close - moveSize * 1.5,
                riskReward: 4.0
            ...
            // Check if price is near this zone
            if (currentPrice >= mitigationZone.low * 0.99 && currentPrice <= mitigationZone.high * 1.01) {
                mitigationBlocks.bullish.push(mitigationZone);
            }
        }
    }
...
// 4. LIQUIDITY GRAB ENTRY - Enter after stop hunt
function findLiquidityGrabEntry(data, liquiditySweep) {
    if (!liquiditySweep || data.length < 10) return null;
    
    const currentPrice = data[data.length - 1].close;
    const recent = data.slice(-5);
    const atr = calculateATR(data, 14);
    
    // Bullish liquidity grab: Price swept lows then reversed up
    if (liquiditySweep.bullish) {
        const entry = currentPrice;
        const stopLoss = liquiditySweep.swingLow - atr * 0.3; // Tight stop below the sweep
        const target = currentPrice + atr * 4.0; // 4R target
        ...
        return {
            ...
            confidence: 90,
            reason: 'Bullish Liquidity Grab - Stop Hunt Reversal'
        };
    }
    
    // Bearish liquidity grab: Price swept highs then reversed down
    if (liquiditySweep.bearish) {
        const entry = currentPrice;
        const stopLoss = liquiditySweep.swingHigh + atr * 0.3; // Tight stop above the sweep
        const target = currentPrice - atr * 4.0; // 4R target
        ...
        return {
            ...
            confidence: 90,
            reason: 'Bearish Liquidity Grab - Stop Hunt Reversal'
        };
    }
...
// 5. FAIR VALUE GAP (FVG) ENTRY - Enter at imbalance zones
function findFVGEntry(data, fvg) {
    if (!fvg || data.length < 10) return { bullish: [], bearish: [] };
    
    const currentPrice = data[data.length - 1].close;
    const fvgEntries = { bullish: [], bearish: [] };
    
    // Bullish FVG: Price returning to gap for support
    fvg.bullish.forEach(gap => {
        const inGap = currentPrice >= gap.low * 0.995 && currentPrice <= gap.high * 1.005;
        
        if (inGap) {
            const gapSize = gap.high - gap.low;
            fvgEntries.bullish.push({
                entry: gap.low + gapSize * 0.5, // Enter at middle of gap
                stopLoss: gap.low - gapSize * 0.5, // Stop below gap
                target: gap.high + gapSize * 5, // 5R target
                riskReward: 5.0,
                confidence: 80,
    ...
    // Bearish FVG: Price returning to gap for resistance
    fvg.bearish.forEach(gap => {
        const inGap = currentPrice >= gap.low * 0.995 && currentPrice <= gap.high * 1.005;
        
        if (inGap) {
            const gapSize = gap.high - gap.low;
            fvgEntries.bearish.push({
                entry: gap.high - gapSize * 0.5, // Enter at middle of gap
                stopLoss: gap.high + gapSize * 0.5, // Stop above gap
                target: gap.low - gapSize * 5, // 5R target
                riskReward: 5.0,
                confidence: 80,
...
// 6. SMART MONEY REVERSAL (SMR) - Institutional reversal pattern
function detectSmartMoneyReversal(data) {
    ...
    // Look for: Strong move → Small consolidation → Reversal
    const last3 = recent.slice(-3);
    const prev5 = recent.slice(-8, -3);
    
    // Calculate momentum
    const recentMomentum = last3[last3.length - 1].close - last3[0].close;
    const prevMomentum = prev5[prev5.length - 1].close - prev5[0].close;
    
    // Bullish SMR: Strong down move, then reversal up
    if (prevMomentum < -atr && recentMomentum > atr * 0.5) {
        const low = Math.min(...prev5.map(c => c.low));
        return {
            type: 'BUY',
            entry: currentPrice,
            stopLoss: low - atr * 0.3,
            target: currentPrice + atr * 5,
            riskReward: 5.0,
            confidence: 85,
            reason: 'Bullish Smart Money Reversal'
        };
    }
    
    // Bearish SMR: Strong up move, then reversal down
    if (prevMomentum > atr && recentMomentum < -atr * 0.5) {
        const high = Math.max(...prev5.map(c => c.high));
        return {
            type: 'SELL',
            entry: currentPrice,
            ...
            riskReward: 5.0,
            confidence: 85,
            reason: 'Bearish Smart Money Reversal'
        };
    }
...
// ENHANCED SIGNAL GENERATION WITH ICT/SMC ENTRY MODELS
function generateEnhancedICTSignal(data, aiPrediction, volumeAnalysis, mtfTrendFilter) {
    // Declare variables outside try block
    let orderBlocks, fvg, liquiditySweep, breakerBlocks;
    let oteEntry, breakerEntry, mitigationEntry, liquidityGrabEntry, fvgEntry, smrEntry;
    
    try {
        // Get all ICT/SMC components with null checks
        orderBlocks = findOrderBlocks(data) || { bullish: [], bearish: [] };
        fvg = findFairValueGaps(data) || { bullish: [], bearish: [] };
        liquiditySweep = detectLiquiditySweep(data) || { bullish: false, bearish: false };
        breakerBlocks = findBreakerBlocks(data, orderBlocks) || { bullish: [], bearish: [] };
        
        // Find optimal entries with null checks
        oteEntry = findOptimalTradeEntry(data, orderBlocks);
        breakerEntry = findBreakerBlockEntry(data, orderBlocks) || { bullish: [], bearish: [] };
        mitigationEntry = findMitigationBlockEntry(data) || { bullish: [], bearish: [] };
        liquidityGrabEntry = findLiquidityGrabEntry(data, liquiditySweep);
        fvgEntry = findFVGEntry(data, fvg) || { bullish: [], bearish: [] };
        smrEntry = detectSmartMoneyReversal(data);
    } catch (error) {
        console.warn('Enhanced ICT signal generation error:', error.message);
        return null;
    }
    
    // Priority order (highest quality entries first):
    // 1. Liquidity Grab (90% confidence, 4R+)
    // 2. Smart Money Reversal (85% confidence, 5R)
    // 3. Breaker Block (85% confidence, 3.5R)
    // 4. FVG Entry (80% confidence, 5R)
    // 5. OTE Entry (75% confidence, 3R)
    
    // Check for high-quality entries
    if (liquidityGrabEntry && liquidityGrabEntry.riskReward >= 3.5) {
        console.log('🎯 LIQUIDITY GRAB ENTRY FOUND:', liquidityGrabEntry.reason);
        // Convert to proper format with targets array
        const entry = liquidityGrabEntry.entry;
        const target = liquidityGrabEntry.target;
        const distance = Math.abs(target - entry);
        
        return {
            type: liquidityGrabEntry.type,
            entry: entry,
            stopLoss: liquidityGrabEntry.stopLoss,
            targets: [
                { price: liquidityGrabEntry.type === 'BUY' ? entry + distance * 0.5 : entry - distance * 0.5, rr: 2.0, percentage: 40 },
                { price: liquidityGrabEntry.type === 'BUY' ? entry + distance * 0.75 : entry - distance * 0.75, rr: 3.0, percentage: 30 },
                { price: target, rr: liquidityGrabEntry.riskReward, percentage: 30 }
            ],
            strength: liquidityGrabEntry.confidence,
            confluence: 18,
            reason: liquidityGrabEntry.reason,
            riskReward: liquidityGrabEntry.riskReward,
            bestRR: liquidityGrabEntry.riskReward
        };
    }
    
    if (smrEntry && smrEntry.riskReward >= 4.0) {
        console.log('🎯 SMART MONEY REVERSAL FOUND:', smrEntry.reason);
        // Convert to proper format with targets array
        const entry = smrEntry.entry;
        const target = smrEntry.target;
        const distance = Math.abs(target - entry);
        
        return {
            type: smrEntry.type,
            entry: entry,
            stopLoss: smrEntry.stopLoss,
            targets: [
                { price: smrEntry.type === 'BUY' ? entry + distance * 0.6 : entry - distance * 0.6, rr: 3.0, percentage: 40 },
                { price: smrEntry.type === 'BUY' ? entry + distance * 0.8 : entry - distance * 0.8, rr: 4.0, percentage: 30 },
                { price: target, rr: smrEntry.riskReward, percentage: 30 }
            ],
            strength: smrEntry.confidence,
            confluence: 15,
            reason: smrEntry.reason,
            riskReward: smrEntry.riskReward,
            bestRR: smrEntry.riskReward
        };
    }
    
    // Check breaker blocks
    if (breakerEntry && breakerEntry.bullish && breakerEntry.bullish.length > 0 && aiPrediction?.signal === 'BUY') {
        const entry = breakerEntry.bullish[0];
        console.log('🎯 BULLISH BREAKER BLOCK ENTRY');
        return {
            type: 'BUY',
            entry: entry.entry,
            stopLoss: entry.stopLoss,
            targets: [
                { price: entry.target * 0.7, rr: 2.5, percentage: 40 },
                { price: entry.target * 0.85, rr: 3.0, percentage: 30 },
                { price: entry.target, rr: entry.riskReward, percentage: 30 }
            ],
            strength: entry.confidence,
            confluence: 12,
            reason: 'Breaker Block Entry'
        };
    }
    
    if (breakerEntry && breakerEntry.bearish && breakerEntry.bearish.length > 0 && aiPrediction?.signal === 'SELL') {
        const entry = breakerEntry.bearish[0];
        console.log('🎯 BEARISH BREAKER BLOCK ENTRY');
        return {
            type: 'SELL',
            entry: entry.entry,
            stopLoss: entry.stopLoss,
            targets: [
                { price: entry.target * 1.3, rr: 2.5, percentage: 40 },
                { price: entry.target * 1.15, rr: 3.0, percentage: 30 },
                { price: entry.target, rr: entry.riskReward, percentage: 30 }
            ],
            strength: entry.confidence,
            confluence: 12,
    ...
    // Check FVG entries
    if (fvgEntry && fvgEntry.bullish && fvgEntry.bullish.length > 0 && aiPrediction?.signal === 'BUY') {
        const entry = fvgEntry.bullish[0];
        console.log('🎯 BULLISH FVG ENTRY');
        return {
            type: 'BUY',
            entry: entry.entry,
            stopLoss: entry.stopLoss,
            targets: [
                { price: entry.target * 0.6, rr: 3.0, percentage: 40 },
                { price: entry.target * 0.8, rr: 4.0, percentage: 30 },
                { price: entry.target, rr: entry.riskReward, percentage: 30 }
            ],
            strength: entry.confidence,
            confluence: 11,
    ...
    if (fvgEntry && fvgEntry.bearish && fvgEntry.bearish.length > 0 && aiPrediction?.signal === 'SELL') {
        const entry = fvgEntry.bearish[0];
        console.log('🎯 BEARISH FVG ENTRY');
        return {
            type: 'SELL',
            entry: entry.entry,
            stopLoss: entry.stopLoss,
            targets: [
                { price: entry.target * 1.4, rr: 3.0, percentage: 40 },
                { price: entry.target * 1.2, rr: 4.0, percentage: 30 },
                { price: entry.target, rr: entry.riskReward, percentage: 30 }
            ],
            strength: entry.confidence,
            confluence: 11,
...
window.generateEnhancedICTSignal = generateEnhancedICTSignal;
...
window.findOptimalTradeEntry = findOptimalTradeEntry;
window.findBreakerBlockEntry = findBreakerBlockEntry;
window.findLiquidityGrabEntry = findLiquidityGrabEntry;
