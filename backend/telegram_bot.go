...
func StartTelegramSignalBot(symbol, strategy string, filterBuy, filterSell bool) error {
	...
	log.Printf("🤖 Telegram bot started - notifications enabled for %s with %s strategy", symbol, strategy)
	log.Printf("ℹ️  Telegram bot is now a messenger only (signals generated by Live Signal Handler)")
	log.Printf("ℹ️  Current filter settings: filterBuy=%v, filterSell=%v", currentFilterBuy, currentFilterSell)
...
func StopTelegramSignalBot() {
	if telegramBot != nil && telegramBot.Running {
		...
		log.Println("🤖 Telegram bot stopped - notifications disabled")
	}
}
...
func (bot *TelegramBot) SendSignal(signal LiveSignalResponse, symbol, strategy string) {
	...
	if !bot.Running {
		log.Printf("ℹ️  Telegram notifications disabled, not sending %s signal", signal.Signal)
		return
	}
